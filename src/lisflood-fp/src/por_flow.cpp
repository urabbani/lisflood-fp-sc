...
NUMERIC_TYPE CalcFPQxPor(int i,int j,States *Statesptr,Pars *Parptr, Solver *Solverptr, Arrays *Arrptr)
{
  NUMERIC_TYPE z0,z1,h0,h1,Sf,hflow,fn,dh,Qlim,Q,alpha;
  NUMERIC_TYPE por0,por1,por;
  int p0,p1;
  int pH0,pH1;
  ...
  if(z0+h0>z1+h1 && h0>Solverptr->DepthThresh) { // Flow from 0->1
    ...
    if(MaskTest(Arrptr->ChanMask[p0],Arrptr->ChanMask[p1]) && hflow>Solverptr->DepthThresh) 
    {
      ...
      if(Parptr->Por_Ident==1)
      {
        ...
        Q=(pow(hflow,(C(5.)/C(3.)))*Sf*Parptr->dy/fn)*por;
      }
...
// SCALING THE FLOW BASED ON THE POROSITY, TJF
NUMERIC_TYPE CalcFPQyPor(int i,int j,States *Statesptr,Pars *Parptr, Solver *Solverptr, Arrays *Arrptr)
{
  NUMERIC_TYPE z0,z1,h0,h1,Sf,hflow,fn,dh,Qlim,Q,alpha;
  NUMERIC_TYPE por0,por1,por;
  int p0,p1;
  ...
  else if(z0+h0<z1+h1 && h1>Solverptr->DepthThresh) 
  {
    ...
    if(MaskTest(Arrptr->ChanMask[p0],Arrptr->ChanMask[p1]) && hflow>Solverptr->DepthThresh) 
    {
      ...
      {
        // flow limiter
        Qlim=Parptr->dA*fabs(dh)/(C(8.)*Solverptr->Tstep);
        if(fabs(Q)>Qlim) 
        {
          if(Q>0) Q=Qlim;
          if(Q<0) Q=-Qlim;
...
NUMERIC_TYPE PorArea(int i,int j,Pars *Parptr, Arrays *Arrptr)
{
  NUMERIC_TYPE h0,por0;
  int p0,pH0;
  NUMERIC_TYPE dAPor;
  int zsz,maxelev,zlev;
