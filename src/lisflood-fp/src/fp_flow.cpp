...
{
  int i,j;
  NUMERIC_TYPE h0,h1,ThreadTS, TmpTstep;
  NUMERIC_TYPE *hptr0,*hptr1,*qptr, *TSptr;
  int *wiptr;
...
NUMERIC_TYPE CalcFPQx(int i,int j,States *Statesptr,Pars *Parptr,Solver *Solverptr,Arrays *Arrptr, NUMERIC_TYPE * TSptr)
{
  NUMERIC_TYPE z0,z1,h0,h1,Sf,hflow,fn,dh,Qlim,Q,alpha;
  int p0,p1,pTQ;
  ...
  { 
    ...
    if(MaskTest(Arrptr->ChanMask[p0],Arrptr->ChanMask[p1]) && hflow>Solverptr->DepthThresh) {
      if(dh<Solverptr->dhlin) 
      {
        ...
        alpha=(pow(hflow,(C(5.0)/C(3.0)))*Parptr->dx_sqrt)/(fn*sqrt(Solverptr->dhlin));
      } 
      else alpha=pow(hflow,(C(5.0)/C(3.0)))/(C(2.)*fn*Sf);

      Q=(pow(hflow,(C(5.0)/C(3.0)))*Sf*Parptr->dy/fn);
      if(Statesptr->adaptive_ts==ON) 
      {
        *TSptr=getmin(*TSptr,(C(0.25)*Parptr->dy*Parptr->dy/alpha));
  ...
  if (Statesptr->voutput==ON)
  {
	  ...
	  else Arrptr->Vx[pTQ]=C(0.0);
  }
  return(Q);
}
//-----------------------------------------------------------------------------------
// CALCULATE FLOODPLAIN (ie NON WIER) FLOW BETWEEN A POINT AND ITS S NEIGHBOUR
NUMERIC_TYPE CalcFPQy(int i,int j,States *Statesptr,Pars *Parptr,Solver *Solverptr,Arrays *Arrptr, NUMERIC_TYPE * TSptr)
{
  NUMERIC_TYPE z0,z1,h0,h1,Sf,hflow,fn,dh,Qlim,Q,alpha;
  int p0,p1,pTQ;
