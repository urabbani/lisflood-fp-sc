...
void ReadCheckpoint(Fnames *Fnameptr, States *Statesptr, Pars *Parptr, Solver *Solverptr, BoundCs *BCptr,ChannelSegmentType *ChannelSegments, Arrays *Arrptr, const int verbose)
{
  int xchk,ychk;
  FILE *check_fp;
  ...
  else		 // go ahead and read data if a file was successfully opened
  {
	...
	if(feof(check_fp)) {
		...
		fread(&Solverptr->itrn_time,sizeof(NUMERIC_TYPE),1,check_fp);
		fread(&Solverptr->t,sizeof(NUMERIC_TYPE),1,check_fp);
		fread(&Solverptr->itCount,sizeof(long),1,check_fp);

		fread(&Parptr->MassTotal,sizeof(NUMERIC_TYPE),1,check_fp);
		fread(&Solverptr->Tstep,sizeof(NUMERIC_TYPE),1,check_fp);
		fread(&Solverptr->MinTstep,sizeof(NUMERIC_TYPE),1,check_fp);
		...
		fread(&Parptr->SaveTotal,sizeof(NUMERIC_TYPE),1,check_fp);
		...
		fread(Arrptr->H,sizeof(NUMERIC_TYPE),Parptr->xsz*Parptr->ysz,check_fp);
		fread(Arrptr->Qx,sizeof(NUMERIC_TYPE),(Parptr->xsz+1)*(Parptr->ysz+1),check_fp);
		fread(Arrptr->Qy,sizeof(NUMERIC_TYPE),(Parptr->xsz+1)*(Parptr->ysz+1),check_fp);
		fread(Arrptr->maxH,sizeof(NUMERIC_TYPE),Parptr->xsz*Parptr->ysz,check_fp);
		fread(Arrptr->maxHtm,sizeof(NUMERIC_TYPE),Parptr->xsz*Parptr->ysz,check_fp);
		fread(Arrptr->initHtm,sizeof(NUMERIC_TYPE),Parptr->xsz*Parptr->ysz,check_fp);
		if(checkv>1) fread(Arrptr->totalHtm,sizeof(NUMERIC_TYPE),Parptr->xsz*Parptr->ysz,check_fp);
		...
		fread(&BCptr->Qin,sizeof(NUMERIC_TYPE),1,check_fp);
		fread(&BCptr->Qout,sizeof(NUMERIC_TYPE),1,check_fp);
		fread(&BCptr->QChanOut,sizeof(NUMERIC_TYPE),1,check_fp);
		fread(&Solverptr->Hds,sizeof(NUMERIC_TYPE),1,check_fp);
		fread(&BCptr->Qpoint_pos, sizeof(NUMERIC_TYPE), 1, check_fp);
		fread(&BCptr->Qpoint_neg, sizeof(NUMERIC_TYPE), 1, check_fp);

		fread(&Parptr->dx,sizeof(NUMERIC_TYPE),1,check_fp);
		fread(&Parptr->dy,sizeof(NUMERIC_TYPE),1,check_fp);
		fread(&Parptr->dA,sizeof(NUMERIC_TYPE),1,check_fp);

		fread(&Parptr->tlx,sizeof(NUMERIC_TYPE),1,check_fp);
		fread(&Parptr->tly,sizeof(NUMERIC_TYPE),1,check_fp);
		fread(&Parptr->blx,sizeof(NUMERIC_TYPE),1,check_fp);
		fread(&Parptr->bly,sizeof(NUMERIC_TYPE),1,check_fp);

		fread(&Solverptr->FArea,sizeof(NUMERIC_TYPE),1,check_fp);
		fread(&Solverptr->vol1,sizeof(NUMERIC_TYPE),1,check_fp);
		fread(&Solverptr->vol2,sizeof(NUMERIC_TYPE),1,check_fp);

		fread(&Solverptr->Qerror,sizeof(NUMERIC_TYPE),1,check_fp);

		if(checkv>2) {
			fread(&Parptr->InfilTotalLoss,sizeof(NUMERIC_TYPE),1,check_fp);
			fread(&Parptr->EvapTotalLoss,sizeof(NUMERIC_TYPE),1,check_fp);
		}
		...
		// main loop for channel segments
			for(chseg=0;chseg<ChannelSegments->N_Channel_Segments;chseg++)
			{
				...
				// read in flows
				fread(csp->ChanQ,sizeof(NUMERIC_TYPE),csp->chsz,check_fp);
				// read in A
				fread(csp->A,sizeof(NUMERIC_TYPE),csp->chsz,check_fp);
				fread(csp->NewA,sizeof(NUMERIC_TYPE),csp->chsz,check_fp);
				// read in junction water depth
				fread(&csp->JunctionH,sizeof(NUMERIC_TYPE),1,check_fp);
        
				if(chseg>0) // trib
				{
					// trib outflow
					fread(&ChannelSegments[csp->Next_Segment].Q_Val[csp->Next_Segment_Loc],sizeof(NUMERIC_TYPE),1,check_fp);
				}
			}
		}

		if(verbose == ON) printf("\n - Computation time so far: %.4" NUM_FMT" hrs, Sim time: %.3" NUM_FMT" of %.3" NUM_FMT" secs\n",(Solverptr->itrn_time/C(3600.0)),Solverptr->t,Solverptr->Sim_Time);
		fclose(check_fp);
		}
	}
...
// Added by Matt Wilson, 25 Nov 2004
// Updated by Tim Fewtrell, February 2009
void WriteCheckpoint(Fnames *Fnameptr, States *Statesptr, Pars *Parptr, Solver *Solverptr, BoundCs *BCptr,ChannelSegmentType *ChannelSegments, Arrays *Arrptr, const int verbose)
{
  FILE *check_fp;
  ChannelSegmentType *csp;  //local pointer to channel segment
  int chseg;	// channel segment loop counter
  ...
  if((check_fp=fopen(Fnameptr->checkpointfilename,"wb"))==NULL) {
    ...
    fwrite(&Solverptr->itrn_time_now,sizeof(NUMERIC_TYPE),1,check_fp);
    fwrite(&Solverptr->t,sizeof(NUMERIC_TYPE),1,check_fp);
    fwrite(&Solverptr->itCount,sizeof(long),1,check_fp);

    fwrite(&Parptr->MassTotal,sizeof(NUMERIC_TYPE),1,check_fp);
    fwrite(&Solverptr->Tstep,sizeof(NUMERIC_TYPE),1,check_fp);
    fwrite(&Solverptr->MinTstep,sizeof(NUMERIC_TYPE),1,check_fp);
    ...
    fwrite(&Parptr->SaveTotal,sizeof(NUMERIC_TYPE),1,check_fp); 
    ...
    if(checkv>1) fwrite(Arrptr->totalHtm,sizeof(NUMERIC_TYPE),Parptr->xsz*Parptr->ysz,check_fp);
    ...
    fwrite(&BCptr->Qin,sizeof(NUMERIC_TYPE),1,check_fp);
    fwrite(&BCptr->Qout,sizeof(NUMERIC_TYPE),1,check_fp);
    ...
    fwrite(&Parptr->dx,sizeof(NUMERIC_TYPE),1,check_fp);
    fwrite(&Parptr->dy,sizeof(NUMERIC_TYPE),1,check_fp);
    fwrite(&Parptr->dA,sizeof(NUMERIC_TYPE),1,check_fp);

    fwrite(&Parptr->tlx,sizeof(NUMERIC_TYPE),1,check_fp);
    fwrite(&Parptr->tly,sizeof(NUMERIC_TYPE),1,check_fp);
    fwrite(&Parptr->blx,sizeof(NUMERIC_TYPE),1,check_fp);
    fwrite(&Parptr->bly,sizeof(NUMERIC_TYPE),1,check_fp);

	fwrite(&Solverptr->FArea,sizeof(NUMERIC_TYPE),1,check_fp);
	fwrite(&Solverptr->vol1,sizeof(NUMERIC_TYPE),1,check_fp);
    fwrite(&Solverptr->vol2,sizeof(NUMERIC_TYPE),1,check_fp);
    fwrite(&Solverptr->Qerror,sizeof(NUMERIC_TYPE),1,check_fp);
    ...
    for(chseg=0;chseg<ChannelSegments->N_Channel_Segments;chseg++)
    {
      ...
      csp=ChannelSegments+chseg;
      // write out flows
      fwrite(csp->ChanQ,sizeof(NUMERIC_TYPE),csp->chsz,check_fp);
	  // write out A
	  fwrite(csp->A,sizeof(NUMERIC_TYPE),csp->chsz,check_fp);
	  fwrite(csp->NewA,sizeof(NUMERIC_TYPE),csp->chsz,check_fp);
      // read in junction water depth
      fwrite(&csp->JunctionH,sizeof(NUMERIC_TYPE),1,check_fp);

      if(chseg>0) // trib
      {
        // trib outflow
        fwrite(&ChannelSegments[csp->Next_Segment].Q_Val[csp->Next_Segment_Loc],sizeof(NUMERIC_TYPE),1,check_fp);;
      }
	}
    }
	fclose(check_fp);
	printf("Checkpointed at %.4" NUM_FMT" hours computation time\n",(Solverptr->itrn_time_now/C(3600.0)));
  }
