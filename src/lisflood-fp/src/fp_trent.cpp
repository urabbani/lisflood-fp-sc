...
NUMERIC_TYPE CalcFPQxRoe(int i,int j,States *Statesptr,Pars *Parptr,Solver *Solverptr,Arrays *Arrptr)
{
  NUMERIC_TYPE z0,z1,h0,h1,hflow,dh=C(0.0),Q,g,dt;
  int p0,p1,pq0;  

  NUMERIC_TYPE dl,hl,hr,s0,hul,hur,hvl,hvr;
  NUMERIC_TYPE dtol;
...
*/
				...
				if (hl*fctm+Arrptr->DEM[p0]  > Arrptr->DEM[p1]  && (hul/hl+pow(g*hl,C(0.5))) >= C(0.)){	/* overtopping */
...
NUMERIC_TYPE CalcFPQyRoe(int i,int j,States *Statesptr,Pars *Parptr,Solver *Solverptr,Arrays *Arrptr)
{
  NUMERIC_TYPE z0,z1,h0,h1,hflow,dh=C(0.0),Q=C(0.0),g,dt;
  int p0,p1,pq0;
    ...
    */
    ...
    //----------------------------------------------------------------------------------------	
    		...
    		if (hl >= dtol && hr >= dtol){	/* both normal */
				...
				if (hr*fctm+Arrptr->DEM[p1] > Arrptr->DEM[p0] && (hvr/hr-pow(g*hr,C(0.5)))<= C(0.)){	/* overtopping */
				...
				{					/* wall */
					...
					else
					{
						...
						//if (hvr <= 0){
							...
							Arrptr->FHy[pq0]=C(0.);//GUSTAVO

							Arrptr->BSHU[p0]=C(0.0) ;//GUSTAVO
							Arrptr->BSHV[p0]=C(0.0) ;//GUSTAVO
							Arrptr->TSHU[p1]=C(0.0) ;//GUSTAVO
							Arrptr->TSHV[p1]=C(0.0) ;//GUSTAVO
					...
					/*
						...
						//Raw formulation from Brufau (Dont add RSH[p0] for avoiding UpdateRoeH)
							...
							if (z0 < z1 + hr*(C(1.)+fabs(ubarra)/cbarra) && fabs(ubarra) < cbarra){
								Arrptr->FHy[pq0]=C(0.5)*(cbarra*(z0-z1)-hr*(fabs(ubarra)+cbarra));
							}
							else{
								Arrptr->FHy[pq0]=0 ;
							}
...
*/
				...
				if (hl*fctm+Arrptr->DEM[p0] > Arrptr->DEM[p1] && (hvl/hl +pow(g*hl,C(0.5)) ) >= C(0.)){	/* overtopping */
		...
		/*				
						...
						// WDC from Brufau et Al    
							...
							if (z1 < z0 + hl*(C(1.)+fabs(ubarra)/cbarra) && fabs(ubarra) < cbarra){
								...
								Arrptr->FHy[pq0]=C(-0.5)*(cbarra*(z1-z0)-hl*(fabs(ubarra)+cbarra));
							}
							else{
								Arrptr->FHy[pq0]=C(0.);
							}
...
/*NUMERIC_TYPE CalcMaxHRoe(Pars *Parptr, Arrays *Arrptr) ... duplicate of CalcMaxH removed JCN
{
	int i,j,p0;
	NUMERIC_TYPE h0, Hmax=C(0.0);
...
void CalcTRoe(Pars *Parptr, Solver *Solverptr, Arrays *Arrptr)
{
	...
	NUMERIC_TYPE locH=C(0.0),locV=C(0.0),locU=C(0.0);
	NUMERIC_TYPE locT,locTx,locTy;
...
void UpdateQsRoe(Pars *Parptr, Solver *Solverptr, Arrays *Arrptr)
{
	...
	NUMERIC_TYPE fn,sf,norm,manning;
	...
	for(j=0;j<Parptr->ysz;j++)
    {
      ...
      {			
		  ...
		  if(Arrptr->H[pc] > Solverptr->MomentumThresh)
		  {
	        ...
	        /* Implicit friction */	
			norm=pow((Arrptr->HU[pc]/Arrptr->H[pc])*(Arrptr->HU[pc]/Arrptr->H[pc])+(Arrptr->HV[pc]/Arrptr->H[pc])*(Arrptr->HV[pc]/Arrptr->H[pc]),C(0.5)) ;		
		    ...
		    manning=pow((pow(fn,C(1.5))/Arrptr->H[pc]+C(0.)*pow(fn,C(1.5))/Parptr->dx),(C(4.)/C(3.))) ;
		    sf=manning*norm  ;
		    Arrptr->HU[pc]*=(C(1.)/(C(1.)+Solverptr->g*Solverptr->Tstep*sf)) ;
		    Arrptr->HV[pc]*=(C(1.)/(C(1.)+Solverptr->g*Solverptr->Tstep*sf)) ;
		  }
		  else{
			Arrptr->HU[pc]=C(0.);
			Arrptr->HV[pc]=C(0.);
