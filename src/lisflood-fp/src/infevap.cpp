...
#include "utility.h"
//-----------------------------------------------------------------------------
// ADVANCED INFILTRATION MODEL
// Implements Green-Ampt infiltration with dynamic soil moisture tracking
void FPInfiltration(Pars *Parptr, Solver *Solverptr, Arrays *Arrptr)
{
  int i, j, p0;
  NUMERIC_TYPE cell_inf, h0;
  
  // Allocate soil moisture arrays if not already allocated
  if (Arrptr->SoilMoisture == NULL) {
    // First time initialization of soil properties
    Arrptr->SoilMoisture.resize(Parptr->xsz * Parptr->ysz);
    Arrptr->SoilSaturation.resize(Parptr->xsz * Parptr->ysz);
    Arrptr->HydraulicConductivity.resize(Parptr->xsz * Parptr->ysz);
    Arrptr->SoilPorosity.resize(Parptr->xsz * Parptr->ysz);
    Arrptr->SuctionHead.resize(Parptr->xsz * Parptr->ysz);
    Arrptr->CumulativeInfiltration.resize(Parptr->xsz * Parptr->ysz);
    
    // Initialize with default values for now - in production these would be loaded from file
    for (j = 0; j < Parptr->ysz; j++) {
      for (i = 0; i < Parptr->xsz; i++) {
        p0 = i + j * Parptr->xsz;
        Arrptr->SoilMoisture[p0] = C(0.2);  // Initial moisture content (fraction)
        Arrptr->SoilSaturation[p0] = C(0.2); // Initial saturation (fraction)
        Arrptr->HydraulicConductivity[p0] = Parptr->InfilRate; // Use global infiltration rate as default Ksat
        Arrptr->SoilPorosity[p0] = C(0.4);  // Default porosity for loamy soil
        Arrptr->SuctionHead[p0] = C(0.1);   // Default suction head (m)
      }
    }
  }

  // Calculate Infiltration using Green-Ampt method
  #pragma omp parallel for private(i, p0, h0, cell_inf)
  for(j=0; j < Parptr->ysz; j++) 
  {
    for(i=0; i < Parptr->xsz; i++) 
    {
      ...
      if(Arrptr->ChanMask[p0] == -1) { //only on non-channel cells
        h0 = Arrptr->H[p0];
        
        if(h0 > Solverptr->DepthThresh) 
        {
          // Get soil properties for this cell
          NUMERIC_TYPE Ksat = Arrptr->HydraulicConductivity[p0];
          NUMERIC_TYPE porosity = Arrptr->SoilPorosity[p0];
          NUMERIC_TYPE current_moisture = Arrptr->SoilMoisture[p0];
          NUMERIC_TYPE psi = Arrptr->SuctionHead[p0];
          NUMERIC_TYPE F = Arrptr->CumulativeInfiltration[p0];
          
          // Calculate available pore space (moisture deficit)
          NUMERIC_TYPE delta_theta = porosity - current_moisture;
          
          if (delta_theta <= C(0.001)) {
            // Soil is saturated, no infiltration occurs
            cell_inf = C(0.0);
          } else {
            // Calculate potential infiltration rate using Green-Ampt equation
            NUMERIC_TYPE inf_rate;
            
            if (F <= C(0.0)) {
              // Initial condition - use saturated conductivity
              inf_rate = Ksat;
            } else {
              // Green-Ampt equation for infiltration rate
              inf_rate = Ksat * (C(1.0) + (psi * delta_theta) / F);
            }
            
            // Calculate infiltration for this time step
            cell_inf = inf_rate * Solverptr->Tstep;
            
            // Limit infiltration by available water and soil moisture capacity
            NUMERIC_TYPE max_possible_inf = delta_theta * Parptr->SoilDepth;
            if (F + cell_inf > max_possible_inf) {
              cell_inf = max_possible_inf - F;
              if (cell_inf < C(0.0)) cell_inf = C(0.0);
            }
            
            // Limit by available water depth
            if (cell_inf > h0) {
              cell_inf = h0;
            }
            ...
            F += cell_inf;
            Arrptr->CumulativeInfiltration[p0] = F;
            
            // Update soil moisture
            NUMERIC_TYPE soil_volume = Parptr->SoilDepth * Parptr->dA;
            NUMERIC_TYPE new_moisture = current_moisture + (cell_inf * Parptr->dA) / soil_volume;
            if (new_moisture > porosity) new_moisture = porosity;
            
            Arrptr->SoilMoisture[p0] = new_moisture;
            Arrptr->SoilSaturation[p0] = new_moisture / porosity;
          }
          
          // Update water depth
          h0 -= cell_inf;
          if(h0 < C(0.0)) {
            cell_inf += h0;  // Correct for any overshoot
            h0 = C(0.0);
            
            // Also correct soil moisture for overshoot
            NUMERIC_TYPE soil_volume = Parptr->SoilDepth * Parptr->dA;
            NUMERIC_TYPE corrected_moisture = Arrptr->SoilMoisture[p0] - ((-h0) * Parptr->dA) / soil_volume;
            Arrptr->SoilMoisture[p0] = corrected_moisture;
            Arrptr->SoilSaturation[p0] = corrected_moisture / Arrptr->SoilPorosity[p0];
          }
          
          Arrptr->H[p0] = h0;
          
          // For mass-balance accounting
...
{
  int i,j;
  NUMERIC_TYPE cell_evap,h0,evap_rate;
...
{
  int i,j, p0;
  NUMERIC_TYPE cell_rain,h0,rain_rate,loc_rainfall_total=C(0.0);
...
//		if(Arrptr->DEM[p0]!=DEM_NO_DATA)
		if (Arrptr->DEM[p0] != Parptr->nodata_elevation)
		{
			h0=Arrptr->H[p0];
			h0+=cell_rain;
...
void FlowDirDEM(Pars *Parptr, Arrays *Arrptr, States *Statesptr, BoundCs *BCptr)
{
	...
	NUMERIC_TYPE north,south,east,west,dx,dy,Routing_Speed;

	//if (Statesptr->latlong==ON)
	//{	
		// create RouteInt array for LatLong grids
		Arrptr->RouteInt=memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz); // always create distributed RouteInt JCN
	//}
	//else
	//{
	...
	for(j=0;j<Parptr->ysz;j++) 
	{
		for(i=0;i<Parptr->xsz;i++) 
		{
			...
			if (Arrptr->DEM[p0] != DEM_NO_DATA)
			{
				int source_index_north = i + (j - 1) * Parptr->xsz;
				int source_index_south = i + (j + 1) * Parptr->xsz;
				int source_index_west = i + (j)* Parptr->xsz - 1;
				int source_index_east = i + (j)* Parptr->xsz + 1;

				int q_index_north = i + j * (Parptr->xsz + 1);
				int q_index_south = i + (j + 1) * (Parptr->xsz + 1);
				int q_index_west = i + j * (Parptr->xsz + 1);
				int q_index_east = i + j * (Parptr->xsz + 1) + 1;
				
				NUMERIC_TYPE dem_this = Arrptr->DEM[p0];
				NUMERIC_TYPE north = (j - 1 >= 0) ? Arrptr->DEM[source_index_north] : DEM_NO_DATA;
				NUMERIC_TYPE south = (j + 1 < Parptr->ysz) ? Arrptr->DEM[source_index_south] : DEM_NO_DATA;
				NUMERIC_TYPE west = (i - 1 >= 0) ? Arrptr->DEM[source_index_west] : DEM_NO_DATA;
				NUMERIC_TYPE east = (i + 1 < Parptr->xsz) ? Arrptr->DEM[source_index_east] : DEM_NO_DATA;
				...
				{
					if (Statesptr->weirs == ON && Arrptr->Weir_Identy[q_index_north] != -1)
					{
					...
					else
					{
						...
						if (Statesptr->dist_routing == OFF) 
							Routing_Speed = Parptr->Routing_Speed; // If distributed routing is OFF use routing speed provide 
						else 
							Routing_Speed = Parptr->Routing_Speed*sqrt((Arrptr->DEM[p0] - north) / dy); // If distributed routing is ON use slope dependent routing speed
				...
				{
					if (Statesptr->weirs == ON && Arrptr->Weir_Identx[q_index_east] != -1)
					{
						Arrptr->FlowDir[p0] = p0;
					}
					else
					{
						...
						if (Statesptr->dist_routing == OFF) 
							...
							Routing_Speed = Parptr->Routing_Speed*sqrt((Arrptr->DEM[p0] - east) / dx); // If distributed routing is ON use slope dependent routing speed
				...
				{
					if (Statesptr->weirs == ON && Arrptr->Weir_Identy[q_index_south] != -1)
					{
						Arrptr->FlowDir[p0] = p0;
					}
					else
					{
						...
						if (Statesptr->dist_routing == OFF)
							...
							Routing_Speed = Parptr->Routing_Speed*sqrt((Arrptr->DEM[p0] - south) / dy); // If distributed routing is ON use slope dependent routing speed
						Arrptr->RouteInt[p0] = dy / Routing_Speed; // calculate RouteInt
					}
				}
				else if(west<=north && west<=east && west<=south && west<Arrptr->DEM[p0]) // flow to west
				{
					if (Statesptr->weirs == ON && Arrptr->Weir_Identx[q_index_west] != -1)
					{
						Arrptr->FlowDir[p0] = p0;
					}
					else
					{
						...
						if (Statesptr->dist_routing == OFF) 
							...
							Routing_Speed = Parptr->Routing_Speed*sqrt((Arrptr->DEM[p0] - west) / dx); // If distributed routing is ON use slope dependent routing speed
						Arrptr->RouteInt[p0] = dx / Routing_Speed; // calculate RouteInt
					}
				}
				else
	...
	{
		Arrptr->Route_dH=memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
		return;
	}
}
//-----------------------------------------------------------------------------
...
{
	int i,j,p0,p1;
	NUMERIC_TYPE cell_rain,rain_rate,h0,h1,z0,z1,flow, flow_fraction;
	NUMERIC_TYPE dh,Sfx,Sfy;
