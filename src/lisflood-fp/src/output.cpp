...
void write_ascfile(const char *root, int SaveNumber, const char *extension, NUMERIC_TYPE *data, NUMERIC_TYPE *dem, int outflag, States *Statesptr, Pars *Parptr)
{
	write_ascfile(root, SaveNumber, extension, data, dem, outflag, Statesptr, Parptr, C(1e-3));
}
...
void write_ascfile(const char *root, int SaveNumber, const char *extension, NUMERIC_TYPE *data, NUMERIC_TYPE *dem, int outflag, States *Statesptr,Pars *Parptr, NUMERIC_TYPE depth_thresh)
{
	...
	write_ascfile(root, SaveNumber, extension, data, dem, outflag, Statesptr, Parptr, depth_thresh, format_specifier);
}
...
NUMERIC_TYPE *data		-	pointer to data
NUMERIC_TYPE *dem		-	pointer to dem
...
*/
{
  ...
  char num_fmt[255];
  strcpy(num_fmt, format_specifier);
  strcat(num_fmt, NUM_FMT);
  strcat(num_fmt, "\t");
  ...
  case 0 :	// normal cell output
    for(j=0;j<Parptr->ysz;j++)
    {
      ...
      {
        fprintf(fp, num_fmt, data[i+j*Parptr->xsz]);
      }
      // output end of line
      fprintf(fp,"\n");
    }
  ...
  case 1 :	// flux ouput - ie cell boundaries not cells
    ...
    // Origin offset by dx*C(0.5) to output fluxes at boundaries when read into GIS (TJF)
    fprintf(fp,"ncols         %i\n",Parptr->xsz+1);
    fprintf(fp,"nrows         %i\n",Parptr->ysz);
    fprintf(fp,"xllcorner     %" NUM_FMT"\n",Parptr->blx-(Parptr->dx/C(2.0)));
    fprintf(fp,"yllcorner     %" NUM_FMT"\n",Parptr->bly);
    fprintf(fp,"cellsize      %" NUM_FMT"\n",Parptr->dx);
    fprintf(fp,"NODATA_value  %" NUM_FMT"\n",NULLVAL);
    ...
    for(j=0;j<Parptr->ysz;j++)
    {
      ...
      {
        fprintf(fp, num_fmt, data[i+j*(Parptr->xsz+1)]);
      }
      // output end of line
      fprintf(fp,"\n");
    }
  ...
  case 2 :	// flux ouput - ie cell boundaries not cells
    ...
    // Origin offset by dy*C(0.5) to output fluxes at boundaries when read into GIS (TJF)
    fprintf(fp,"ncols         %i\n",Parptr->xsz);
    fprintf(fp,"nrows         %i\n",Parptr->ysz+1);
    fprintf(fp,"xllcorner     %" NUM_FMT"\n",Parptr->blx);
    fprintf(fp,"yllcorner     %" NUM_FMT"\n",Parptr->bly-(Parptr->dx/C(2.0)));
    fprintf(fp,"cellsize      %" NUM_FMT"\n",Parptr->dx);
    fprintf(fp,"NODATA_value  %" NUM_FMT"\n",NULLVAL);
  ...
  case 3 :	// special output for water elevation (add DEM to data)
    for(j=0;j<Parptr->ysz;j++)
    {
      for(i=0;i<Parptr->xsz;i++)
      {
        ...
        //if(data[i+j*Parptr->xsz]<=C(0.01)) // set to null for very shallow depth
		if (data[i + j*Parptr->xsz] <= depth_thresh) // TFD update to use same default value as DepthThresh
        {
          // PB: Fixed bug in generation of elev files so these are only created when H<=C(0.01) (not H<=0).  
...
*/
{
	...
	{
	case 0:	// normal cell output
		for (j = 0; j < (2 * Parptr->ysz);j++)//4
		{
			for (i = 0; i < (2 * Parptr->xsz);i++)//4
			{
				fprintf(fp, num_fmt, data[i + j * 2 * Parptr->xsz]);//4
			}
			// output end of line
			fprintf(fp, "\n");
		}
			  ...
			  for(j=0;j<csptr->chsz;j++)
				{
				  ...
				  if(chseg!=0 && j==csptr->chsz-1)
				  {
					...
					fprintf(fp,"%-10.3" NUM_FMT" %10.3" NUM_FMT"", Parptr->blx + (csptr->ChanX[j] * Parptr->dx) + (Parptr->dx/2), Parptr->bly + (Parptr->ysz*Parptr->dx) - ((csptr->ChanY[j] * Parptr->dx) + (Parptr->dx/2))); 
					fprintf(fp," %10.3" NUM_FMT" %10.3" NUM_FMT"", csptr->Chainage[j], csptr->ChanWidth[j]); 
					fprintf(fp," %10.5" NUM_FMT" %10.8" NUM_FMT"", csptr->ChanN[j], csptr->Shalf[j]*fabs(csptr->Shalf[j])); 
					fprintf(fp," %10.3" NUM_FMT" %10.3" NUM_FMT"", csptr->BankZ[j], csptr->JunctionDEM);
					fprintf(fp," %10.3" NUM_FMT" %10.3" NUM_FMT"", csptr->JunctionDEM+csptr->JunctionH, csptr->JunctionH);
					fprintf(fp," %10.3" NUM_FMT"", csptr->ChanQ[j]); 
					if(Statesptr->debugmode==ON) fprintf(fp," %10i %10.3" NUM_FMT"",csptr->Q_Ident[j],csptr->Q_Val[j]);
					fprintf(fp,"\n"); 
				  }
				  else                       
				  {
					...
					fprintf(fp," %10.3" NUM_FMT" %10.3" NUM_FMT"", csptr->Chainage[j], csptr->ChanWidth[j]); 
					fprintf(fp," %10.5" NUM_FMT" %10.8" NUM_FMT"", csptr->ChanN[j], csptr->Shalf[j]*fabs(csptr->Shalf[j])); 
					fprintf(fp," %10.3" NUM_FMT" %10.3" NUM_FMT"", csptr->BankZ[j], Arrptr->DEM[pi+pj*Parptr->xsz]);
					fprintf(fp," %10.3" NUM_FMT" %10.3" NUM_FMT"", Arrptr->DEM[pi+pj*Parptr->xsz]+Arrptr->H[pi+pj*Parptr->xsz], Arrptr->H[pi+pj*Parptr->xsz]);
					fprintf(fp," %10.3" NUM_FMT"", csptr->ChanQ[j]); 
					if(Statesptr->debugmode==ON) fprintf(fp," %10i %10.3" NUM_FMT"",csptr->Q_Ident[j],csptr->Q_Val[j]);
					fprintf(fp,"\n");
				  }
...
void debugfileoutput(Fnames *Fnameptr,States *Statesptr,Pars *Parptr,Arrays *Arrptr)
{
  ...
  {
	  TempChanMask = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);

    for(j=0;j<Parptr->ysz;j++)
    {
      for(i=0;i<Parptr->xsz;i++)
      {
		  TempChanMask[i + j*Parptr->xsz] = (NUMERIC_TYPE)Arrptr->ChanMask[i + j*Parptr->xsz];
      }
    }
    ...
    for(j=0;j<Parptr->ysz;j++)
    {
      for(i=0;i<Parptr->xsz;i++)
      {
		  TempChanMask[i + j*Parptr->xsz] = (NUMERIC_TYPE)Arrptr->SegMask[i + j*Parptr->xsz];
      }
    }

    write_ascfile(Fnameptr->resrootname,-1,".segmask",TempChanMask,Arrptr->DEM,0,Statesptr,Parptr);

	memory_free_legacy(&TempChanMask);
...
void write_binrasterfile(const char *root, int SaveNumber, const char *extension, NUMERIC_TYPE *data, NUMERIC_TYPE *dem, int outflag, States *Statesptr, Pars *Parptr)
{
	write_binrasterfile(root, SaveNumber, extension, data, dem, outflag, Statesptr, Parptr, C(1e-3));
}
...
// MT new multi purpose version - eliminates a lot of repetition
// and implements new filenaming
void write_binrasterfile(const char *root, int SaveNumber, const char *extension, NUMERIC_TYPE *data, NUMERIC_TYPE *dem, int outflag, States *Statesptr, Pars *Parptr, NUMERIC_TYPE depth_thresh)
/*
...
{
  int i,j,tempi;
  NUMERIC_TYPE temp, no_data=-9999;
  FILE *fp;
  char fnam[800];
  //char tmp_sys_com[255];
  ...
  //{
	...
	//    fwrite (ptr, NUMERIC_TYPE , sizeof(buffer) , fp );
    //}
  //} 
  if(outflag==0 || outflag==3) 
  {
	...
	fwrite (&Parptr->blx, sizeof(NUMERIC_TYPE), 1, fp );
	fwrite (&Parptr->bly, sizeof(NUMERIC_TYPE), 1, fp );
	fwrite (&Parptr->dx , sizeof(NUMERIC_TYPE), 1, fp );
	fwrite (&no_data    , sizeof(NUMERIC_TYPE), 1, fp );
  }
  ...
  {
  case 0 :	// normal cell output
	fwrite(data,sizeof(NUMERIC_TYPE),Parptr->xsz*Parptr->ysz,fp);
  ...
  case 1 :	// flux ouput - ie cell boundaries not cells
    ...
    fwrite (&Parptr->ysz, sizeof(int) , 1, fp );
    temp = Parptr->blx-(Parptr->dx/C(2.0));
    fwrite (&temp, sizeof(NUMERIC_TYPE) , 1, fp );
    fwrite (&Parptr->bly, sizeof(NUMERIC_TYPE) , 1, fp );
    fwrite (&Parptr->dx, sizeof(NUMERIC_TYPE) , 1, fp );
    fwrite (&no_data, sizeof(NUMERIC_TYPE) , 1, fp );

    fwrite (data,sizeof(NUMERIC_TYPE),(Parptr->xsz+1)*(Parptr->ysz+1),fp);
  ...
  case 2 :	// flux ouput - ie cell boundaries not cells
    ...
    fwrite (&tempi, sizeof(int) , 1, fp );
    fwrite (&Parptr->blx, sizeof(NUMERIC_TYPE) , 1, fp );
    temp = Parptr->bly-(Parptr->dx/C(2.0));
    fwrite (&temp, sizeof(NUMERIC_TYPE) , 1, fp );
    fwrite (&Parptr->dx, sizeof(NUMERIC_TYPE) , 1, fp );
    fwrite (&no_data, sizeof(NUMERIC_TYPE) , 1, fp );  
  ...
  case 3 :	// special output for water elevation (add DEM to data)
    for(j=0;j<Parptr->ysz;j++)
    {
      for(i=0;i<Parptr->xsz;i++)
      {
        ...
        if(data[i+j*Parptr->xsz]<=depth_thresh) // set to null for very shallow depth
        {
          // PB: Fixed bug in generation of elev files so these are only created when H<=C(0.01) (not H<=0).  
          // Without this you get significant lateral curvature caused by thin films of water ponding in drying elements.
          temp=NULLVAL;
...
void write_ascfile_SGCf(char *root, int SaveNumber, char *extension, NUMERIC_TYPE *data, NUMERIC_TYPE *SGCbfH, States *Statesptr,Pars *Parptr)
/*
Purpose: writes data to a standard ascii file format
Parameters:
char *root		-	root of filename
...
char *extension	-	filename extension text
NUMERIC_TYPE *data		-	pointer to data
NUMERIC_TYPE *SGCbfH		-	pointer to SGC bankful depth
int outflag		-	flag (if 0 = normal, if 1 or 2 indicated fluxes, if 3 indicates special option Water elev ouput DEM+H)
States *Statesptr - pointer to States structure
Pars *Parptr - pointer to Parameters structure
*/
  ...
  else
  {
    ...
    for(j=0;j<Parptr->ysz;j++)
    {
      for(i=0;i<Parptr->xsz;i++)
      {
		...
		if (temp <= C(0.001)) temp = C(0.0);
        fprintf(fp,"%.3" NUM_FMT"\t",temp);
      }
      // output end of line
      fprintf(fp,"\n");
    }
...
void write_binrasterfile_SGCf(char *root, int SaveNumber, char *extension, NUMERIC_TYPE *data, NUMERIC_TYPE *SGCbfH, States *Statesptr,Pars *Parptr)
/*
Purpose: writes data that would be in an ascii raster to a binary file format
Parameters:
char *root		-	root of filename
...
void write_regular_output( Fnames *Fnameptr, Solver *Solverptr, States *Statesptr, Pars *Parptr, Arrays *Arrptr, SGCprams *SGCptr  )
{
	...
	NUMERIC_TYPE w0, w1, A0, A1, zb0, zb1, cn, bf0, bf1, h0, h1, g, dt, m, dh, Sf, R, qc, hflow, Qc;
	NUMERIC_TYPE *SGCVx, *SGCVy, *SGCvoutput;
    ...
    if(Statesptr->save_depth==ON)
    {
		...
		if (Statesptr->SGC==ON)
		{
			...
			if (Statesptr->binary_out==ON) write_binrasterfile_SGCf(Fnameptr->resrootname,Parptr->SaveNo, ".wdfpb", Arrptr->H, Arrptr->SGCbfH, Statesptr,Parptr); 
    ...
    if(Statesptr->save_elev==ON)
    {
	  ...
	  if (Statesptr->SGC==ON)
	  {
		  if (Statesptr->binary_out == ON) write_binrasterfile(Fnameptr->resrootname, Parptr->SaveNo, ".elevb", Arrptr->H, Arrptr->SGCz, 3, Statesptr, Parptr, Solverptr->DepthThresh);
		  else write_ascfile(Fnameptr->resrootname,Parptr->SaveNo,".elev",Arrptr->H,Arrptr->SGCz,3,Statesptr,Parptr, Solverptr->DepthThresh);
	  }
	  else
	  {
		  // output binary of ascii rasters
		  if (Statesptr->binary_out == ON) write_binrasterfile(Fnameptr->resrootname, Parptr->SaveNo, ".elevb", Arrptr->H, Arrptr->DEM, 3, Statesptr, Parptr, Solverptr->DepthThresh);
		  else write_ascfile(Fnameptr->resrootname, Parptr->SaveNo, ".elev", Arrptr->H, Arrptr->DEM, 3, Statesptr, Parptr, Solverptr->DepthThresh);
	  }
	}
	...
	{
		// create variables 
		SGCVx = memory_allocate_zero_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
		SGCVy = memory_allocate_zero_numeric_legacy((Parptr->xsz + 1)*(Parptr->ysz + 1));
		SGCvoutput = memory_allocate_zero_numeric_legacy(Parptr->xsz*Parptr->ysz);
			...
			if (w0>C(0.0) && w1>C(0.0))
			{
				...
				if(hflow>Solverptr->DepthThresh) 
				{
					...
					if (A0 < A1) // select the appropriate slope and area based on the smallest area
					{
						...
						Qc =  (qc-g*A0*dt*Sf) / (1+dt*g*cn*fabs(qc) / (pow(R,C(4.0)/C(3.0))*A0) );
						SGCVx[pq0] = Qc/A0;
					}
					else 
					{
						...
						// Calculate channel flow using inertial wave equation --- in this case Qxold will be in m3s-1 not m2s-1
						Qc =  (qc-g*A1*dt*Sf) / (1+dt*g*cn*fabs(qc) / (pow(R,C(4.0)/C(3.0))*A1) );
						SGCVx[pq0] = Qc/A1;
					}
				}
				else SGCVx[pq0] = C(0.0); // make sure previous flux is set to zero
			}
		...
		for(j=0;j<Parptr->ysz-1;j++) for(i=0;i<Parptr->xsz;i++)
		{
      		...
      		if (w0>C(0.0) && w1>C(0.0))
			{
				...
				else SGCVy[pq0] = C(0.0); // make sure previous flux is set to zero
			}
		}
    ...
    // output regular files - this is written at the end no need to write/overwrite every time
