...
#define LF_VersionMajor 8
#define LF_VersionMinor 1
#define LF_VersionInc   0
#define LF_CheckVersion 0
...
Ver  Date         Details
---  ----         -------------------------------------------------------------------------
8.1  Nov 23 2020  Merged changes from v7.1 (d8subgrid) into v8 (PFU)
8.0  Jul 24 2020  FV1 and DG2 Solvers added (JS)
7.1  Oct 01 2019  D8 directions option for subgrid channels. (PFU)
7.0  Sep 28 2017  Super-grid model (JCN)
6.3  Now 21 2016  Froude flow limiter added to SGC calcQ as solver option _CALCULATE_Q_MODE = 1 to activate (JCN)
6.2	 Sep 09 2016  Reservoir Model added (FEOL)
6.1	 Nov 16 2015  Distributed rainfall added (AS)
6.0  Dec 22 2014  Optomized version (Toby Dunne) * corrected routing  implementation * diffusive switch * netCDF
...
----         -------  -----------------------------------------------------------------------
JUL 31 2020  8.0.3    rearranged dynamic rain in SGM for greater efficiency (JCN)
JUL 31 2020  8.0.2    Fixes to dynamic rain in SGM (JCN)
JUL 24 2020  8.0.1    dynamic rainfall added st SGM fast (JCN)
JAN 14 2020  7.0.8    Bug fix for HVAR and HFIX where SBG is bigger than chell size (JCN)
DEC 18 2019  7.0.7    Minor changes to supergrid model to work with rainfall (JCN)
JUN 21 2019  7.0.6    Added distributed infiltration to SG model (JCN)
Jan 03 2019  7.0.5    Added wet dry bounds tracking to supergrid model (JCN)
Oct 04 2017  7.0.4    Added link lisk file read option to super grid model (JCN)
Oct 03 2017  7.0.3    First version of super-grid model single core only (JCN)
Sep 29 2017  7.0.1    Groundwork for super-grid model added test case created (JCN)
Sep 28 2017  6.3.5    Fix bug in case where SGCbed is above DEM (JCN)
Aug 09 2017  6.3.4    Increased precision on ascii header output (important for high resolution latlong models) (JCN)
Jun 14 2017  6.3.3    Option to set a save max interval added (JCN)
Jun 13 2017  6.3.2    Added ability to export max at save intervals (JCN)
Nov 24 2016  6.3.1    Fixed startfile inport to work with latlong (JCN)
Apr 22 2016  6.1.4	  Fixed bug-Stage and Discharge output now using massint value when using tstart (LH)
Apr 11 2016  6.1.3    Added ability to use spatially-varying evaporation from a netcdf file (KA)
Feb 23 2016  6.1.2    Fixed max weir flow calculation. (AS)
Nov 16 2015  6.1.1    Rainfall and Evap mass balance calculation bug fix (JCN). Max depth only keyword added (AS)
Sep 10 2015  6.0.5    Fixed SGC CalculateQ to not use getmin(R,10.0) (JCN)
Feb 26 2015  6.0.4    Fix overpass writing with write_sum_grid
Dec 27 2014  6.0.3    Option to disable netcdf build.
Nov 13 2014  5.9.31   Sub grid flow - ensure no flow where DEM = no data (Toby Dunne)
Nov 07 2014  5.9.30   HVAR HFIX boundary conditions correct dh when boundary has a sub-grid-channel (Toby Dunne)
Nov 06 2014  5.9.29   Consistently switch modes, depending on when sub grid Arrptr->SGCwidth[p0] >= C(0.5)*(Arrptr->dx[p0]+Arrptr->dy[p0]) (evap and update h)
Nov 05 2014  5.9.28   Weir bug fixed - CalcWeirQy was setting QxSGold should be QySGold (Toby Dunne)
Nov 03 2014  5.9.27   Fix OpenMP threading issue with Qin and Qout update. (Toby Dunne)
Sep 25 2014  5.9.26   Fix for LatLong support, boundary conditions now uses correct width/length (Toby Dunne)
                      waterdepth (.wdfp) output zero if below depth_thresh (vs previous hard coded constant 0.001)
                      waterdepth (.wdfpb) output zero if below depth_thresh (vs previous hard coded 0.0)
                      evaporation - stop at depth_thresh as per previous model behaviour.
                      Elevation file output use depth_thresh from parameter file/command line.
Aug 21 2014  5.9.25   add "minutes" units to all timeseries inputs for consistency and also warn user if units not recognised (usually because comment line is missing)  (MT)
Aug 21 2014  5.9.24   split point inputs into positive and negative so that they can be added to Qin and Qout correctly (source and sinks)  (MT)
Aug 21 2014  5.9.23   modify so that debug dem (ie dem+river beds) is output by default for correct visualisation of river depths (MT)
Aug 21 2014  5.9.22   Fix minor bug in utility.h which only affected compiled versions from MSV studio 2012 (MT)
Jul 29 2014  5.9.21   Evaporation allow cell to completly dry, Evaporation only update volume (not h), maxH only track when h above DepthThresh, track initial inundation at DepthThresh (replace 0.01). (TFD)
Jul 23 2014  5.9.20   Correct an issue where SGCBed was not read, levees incorectly overwrote value when turned off. (TFD)
Jul 23 2014  5.9.19   Correctly reset Q old and Q old sub-grid to zero when cell heights less than depth threshold. Output elevation use consistent depth threshold. (TFD)
Jul 18 2014  5.9.18   Reading parameters file - read line better hendling of commented out params (TFD)
Jul 18 2014  5.9.17   Fix QVAR boundary - previously was not reading the bdi flow (TFD)
Jul 11 2014  5.9.16   Reading parameters file - check input (TFD)
Jul 11 2014  5.9.15   Bug Fixes - Evaporation, SGC_Evaporation impact on results. Bug fix IO file closing and freeing memory. Performance remove unnecessary regular output. (TFD)
Jul 08 2014  5.9.14   Corrected point HVAR and HFIX boundary (removed duplicated update from 5.9.12) (TFD)
Jul 08 2014  5.9.13   Update code to enable switch between double and float with compiler switch. Fix FloodArea OpenMP initialization. scanf and printf character codes fixed. Updated loop over grid to match memory alignment. (TFD)
May 20 2014  5.9.12   Code added to spatially alter SGCbfH if required. Support for levees. (NQ)
